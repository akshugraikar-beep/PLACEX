...
const EmployeeSidebar = ({ isExpanded, setIsExpanded }) => {
  ...
  const sidebarStructure = [
    { icon: LayoutDashboard, label: "HR Dashboard",  path: "/dashboard/employee",              type: "link" },
    {
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between flex-shrink-0 p-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          ...
          <button onClick={() => setIsExpanded(p => !p)} className="text-slate-500 hover:text-white transition-colors flex-shrink-0">
            ...
            if (item.type === "section") {
              ...
              return (
                <div key={item.label} className="px-3 mb-1">
                  <button className={`w-full flex items-center gap-3 py-2.5 rounded-xl mb-1 transition-all ${isExpanded ? "px-3" : "justify-center"} text-slate-500 hover:text-slate-300`}
                    ...
                    <div className={`pl-4 border-l overflow-hidden transition-all duration-300 ${isOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}
                      ...
                      {filteredChildren.map(child => (
                        <NavLink key={child.label} to={child.path}
                          className={({ isActive }) => `flex items-center gap-2.5 py-2 px-3 rounded-xl mb-0.5 text-sm transition-all ${isActive ? "text-violet-300 border border-violet-500/20" : "text-slate-500 hover:text-slate-300"}`}
                          style={({ isActive }) => isActive ? { background: "rgba(124,58,237,0.15)" } : {}}
                          onClick={() => isMobile && setIsExpanded(false)}>
                          <child.icon className="w-3.5 h-3.5 flex-shrink-0" /><span>{child.label}</span>
                        </NavLink>
                      ))}
                    </div>
                  )}
            ...
            if (item.type === "link" && item.label.toLowerCase().includes(searchQuery.toLowerCase())) {
              return (
                <div key={item.label} className="relative group px-3">
                  <NavLink to={item.path} end={item.path === "/dashboard/employee"}
                    className={({ isActive }) => `flex items-center gap-3 py-2.5 rounded-xl mb-1 transition-all ${isExpanded ? "px-3" : "justify-center"} ${isActive ? "text-white" : "text-slate-400 hover:text-white"}`}
                    style={({ isActive }) => isActive ? { background: "linear-gradient(135deg,#7c3aed,#4f46e5)", boxShadow: "0 0 12px rgba(124,58,237,0.35)" } : {}}
                    onClick={() => isMobile && setIsExpanded(false)}>
                    <item.icon className="w-4 h-4 flex-shrink-0" />
                    {isExpanded && <span className="text-sm font-medium truncate">{item.label}</span>}
                  </NavLink>
                  {!isExpanded && !isMobile && (
                    <div className="absolute left-full top-1/2 -translate-y-1/2 ml-2 px-2 py-1 bg-slate-900 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap shadow-xl z-50 border border-white/10">{item.label}</div>
                  )}
                </div>
              );
            }
          ...
          <div className="p-4 flex-shrink-0" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
            <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl"
              ...
              <div>
                ...
                <p className="text-xs text-slate-600">PlaceX HR Portal</p>
              </div>
            </div>
          </div>
        )}
