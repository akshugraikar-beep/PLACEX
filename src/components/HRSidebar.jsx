...
const HRSidebar = ({ isExpanded, setIsExpanded }) => {
  ...
  const sidebarStructure = [
    { icon: LayoutDashboard, label: "HR Dashboard", path: "/dashboard/hr", type: "link" },
    {
      icon: Users, label: "Talent", type: "section", sectionKey: "talent",
      children: [
        { icon: Users,      label: "Candidate Management", path: "/dashboard/hr/candidates",  type: "link" },
        { icon: UserCheck,  label: "Applicants Tracker",   path: "/dashboard/hr/applicants",  type: "link" },
        { icon: Brain,      label: "AI Analysis",          path: "/dashboard/hr/ai-analysis", type: "link" },
        { icon: Star,       label: "Shortlisted",          path: "/dashboard/hr/shortlisted", type: "link" },
      ],
    },
    {
    ...
    },
    {
      ...
      children: [
        { icon: TrendingUp, label: "HR Analytics",      path: "/dashboard/hr/hr-analytics", type: "link" },
        { icon: FileText,   label: "Reports",            path: "/dashboard/hr/reports",      type: "link" },
        { icon: TrendingUp, label: "Company Insights",   path: "/dashboard/hr/insights",     type: "link" },
      ],
    },
    { icon: Building2, label: "Collaboration", path: "/dashboard/hr/collaboration", type: "link" },
    { icon: User,      label: "Profile",       path: "/dashboard/hr/profile",       type: "link" },
    { icon: Settings,  label: "Settings",      path: "/dashboard/hr/settings",      type: "link" },
  ...
  return (
    <>
      ...
      <aside
        ...
        style={{
          background: "linear-gradient(180deg, #0a0a1e 0%, #0d0d2b 100%)",
      ...
      >
        ...
        <div className="flex items-center justify-between flex-shrink-0 p-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          {isExpanded ? (
            <div className="flex items-center gap-2.5 overflow-hidden">
              ...
              <div>
                ...
                <p className="text-xs" style={{ color: "#7c3aed" }}>HR Portal</p>
              </div>
            </div>
          ) : (
          ...
          <button onClick={toggleSidebar} className="text-slate-500 hover:text-white transition-colors flex-shrink-0">
        ...
        <nav className="flex-1 overflow-y-auto overflow-x-hidden pt-3 pb-4" style={{ scrollbarWidth: "thin", scrollbarColor: "#1e293b transparent" }}>
          {sidebarStructure.map((item) => {
                    ...
                    <div className={`pl-4 border-l overflow-hidden transition-all duration-300 ${isOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}
                      ...
                      {filteredChildren.map(child => (
                        <NavLink key={child.label} to={child.path}
                          ...
                          style={({ isActive }) => isActive ? { background: "rgba(124,58,237,0.15)" } : {}}
            ...
            if (item.type === "link" && item.label.toLowerCase().includes(searchQuery.toLowerCase())) {
              return (
                <div key={item.label} className="relative group px-3">
                  <NavLink to={item.path} end={item.path === "/dashboard/hr"}
                    className={({ isActive }) =>
                      `flex items-center gap-3 py-2.5 rounded-xl mb-1 transition-all ${isExpanded ? "px-3" : "justify-center"} ${
                        isActive
                          ? "text-white"
                    ...
                    style={({ isActive }) => isActive
                      ? { background: "linear-gradient(135deg,#7c3aed,#4f46e5)", boxShadow: "0 0 12px rgba(124,58,237,0.35)" }
                      : {}}
                    onClick={() => isMobile && setIsExpanded(false)}
                  >
                    <item.icon className="w-4 h-4 flex-shrink-0" />
            ...
            if (item.type === "button" && item.label.toLowerCase().includes(searchQuery.toLowerCase())) {
              return (
                <div key={item.label} className="relative group px-3">
                  <button onClick={() => { item.onClick?.(); if (isMobile) setIsExpanded(false); }}
                    className={`w-full flex items-center gap-3 py-2.5 rounded-xl mb-1 text-slate-500 hover:text-white transition-all ${isExpanded ? "px-3" : "justify-center"}`}>
                    <item.icon className="w-4 h-4 flex-shrink-0" />
                    {isExpanded && <span className="text-sm font-medium truncate">{item.label}</span>}
                  </button>
                  {!isExpanded && !isMobile && (
          ...
          <div className="p-4 flex-shrink-0" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
            <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl"
              ...
              <div className="overflow-hidden">
                ...
                <p className="text-xs text-slate-600 truncate">PlaceX HR Portal</p>
              </div>
            </div>
          </div>
        )}
