...
const ResumeATS = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Backend returns errors under 'error' key, not 'message'
      if (err.response?.data?.error) {
        const backendError = err.response.data.error;
        if (backendError.toLowerCase().includes("no selectable text") || backendError.toLowerCase().includes("scanned")) {
          errorMessage = `❌ Your PDF appears to be a scanned/image-based file with no readable text. Please use a text-based PDF (where you can highlight/copy text) or a .txt file instead.`;
        } else {
        ...
        errorMessage = `❌ Your PDF appears to be a scanned/image-based file. Please upload a text-based PDF or a .txt file.`;
      } else if (err.response?.status === 400) {
        errorMessage = `❌ Invalid file or missing job description. Please check your inputs.`;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 dark:bg-gradient-to-br dark:from-slate-900 dark:via-blue-900 dark:to-indigo-900">
      ...
      <div className="relative max-w-7xl mx-auto px-6 py-12 space-y-12">
        ...
        {/* Analysis Results */}
        {analysis && (
          <div ref={resultsRef} className="space-y-12 animate-in fade-in duration-1000">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              ...
              {/* Parseability Score */}
              {(() => {
                // Compute parseability client-side from resumeChars since scorer omits it from factors
                const chars = analysis.resumeChars ?? 0;
                const parseScore = chars < 300 ? 10 : chars < 800 ? 50 : 100;
                const parseNote = chars < 300
                  ? "Very little selectable text — possible scanned PDF."
                  : chars < 800
                  ? "Low text volume — consider a more detailed resume."
                  : "Good amount of selectable text.";
                return (
                  <ScoreCard
                    title="Parseability"
                    score={parseScore}
            ...
            {/* Gemini AI Insights - Only show when Gemini succeeded */}
                      ...
                      <div className="bg-red-50 rounded-lg p-4 dark:bg-red-900/20">
                        ...
                        <ul className="space-y-1 text-sm text-red-700 dark:text-red-200">
                          {analysis.geminiAnalysis.weaknesses?.map((weakness, idx) => (
                            <li key={idx} className="flex items-start gap-2">
                              <span className="text-red-500 dark:text-red-300 mt-1">•</span>
                              {weakness}
                            </li>
                      ...
                      <div className="bg-blue-50 rounded-lg p-4 dark:bg-blue-900/10">
                        ...
                        <ul className="space-y-1 text-sm text-blue-700 dark:text-blue-200">
                          {analysis.geminiAnalysis.suggestions?.map((suggestion, idx) => (
                            <li key={idx} className="flex items-start gap-2">
                              <span className="text-blue-500 dark:text-blue-300 mt-1">•</span>
                              {suggestion}
                            </li>
